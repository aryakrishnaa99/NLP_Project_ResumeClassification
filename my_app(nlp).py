...
st.markdown('<style>h1{color: Purple;}</style>', unsafe_allow_html=True)  #we will create page intro like title and subheader
st.subheader('Welcome to Resume Classification App')

                                                     #Here we extract just file name and skills in the resume column with the skill.csv file
# FUNCTIONS                                          #Also pre-process skill data
...
#Initialization: Initialize empty lists for storing file names, predicted profiles, and extracted skills.
...
# MAIN CODE
import pickle as pk
model = pk.load(open(r'model_NB.pkl', 'rb'))
Vectorizer = pk.load(open(r'vector.pkl', 'rb'))

# To provide space for file upload on the app
upload_file = st.file_uploader('Upload Your Resumes', type= ['docx','pdf'],accept_multiple_files=True)

for doc_file in upload_file:
    if doc_file is not None:
        filename.append(doc_file.name)
        cleaned = preprocess(display(doc_file)) #pre-process the text data
        prediction = model.predict(Vectorizer.transform([cleaned]))[0] #use vector file and created model for prediction
        predicted.append(prediction)
        extText = getText(doc_file)
        skills.append(extract_skills(extText))

          #If there are predictions:
          #Populate a DataFrame with the file names, predicted profiles, and extracted skills.
...
if len(predicted) > 0:
 ...
 # To create a dropdown menu for selecting the files according to resume type/classification
select = ['PeopleSoft','SQL Developer','React JS Developer','Workday']
st.subheader('Select as per Requirement')
option = st.selectbox('Fields',select) #this is optional
